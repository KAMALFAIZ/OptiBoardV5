...
def send_whatsapp_message(
    ...
    if not all([account_sid, auth_token, from_number, to_number]):
        ...
        return False
    try:
        # Normaliser les numéros au format WhatsApp
        ...
        if sid:
            logger.info(f"WhatsApp: message envoyé sid={sid} status={status}")
            return True
...
def test_whatsapp_connection(account_sid: str, auth_token: str) -> dict:
    """Teste les credentials Twilio en récupérant les infos du compte."""
    try:
        credentials = base64.b64encode(f"{account_sid}:{auth_token}".encode()).decode()
