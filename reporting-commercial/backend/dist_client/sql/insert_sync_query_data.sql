...
-- Valeurs informations libres - EAV (Order: 2b)
INSERT INTO ETL_Tables_Config (name, source_query, target_table, primary_key, sync_type, timestamp_column, priority, batch_size, sort_order, enabled)
VALUES (N'Valeurs informations libres', N'__INFO_LIBRES_VALUES__',
N'Info_Libres_Valeurs', N'CB_File,entity_key,CB_Name', 'full', NULL, 'high', 50000, 2, 1);

-- Correspondance cle tiers infos libres : CT_Num <-> cbMarq de F_COMPTET (Order: 2c, ADDITIF)
-- Permet de rattacher les infos libres des tiers (F_COMPTET) aux tables Clients/Fournisseurs.
INSERT INTO ETL_Tables_Config (name, source_query, target_table, primary_key, sync_type, timestamp_column, priority, batch_size, sort_order, enabled)
VALUES (N'Correspondance clé tiers', N'SELECT CT_Num AS [Code tiers], CAST(cbMarq AS NVARCHAR(50)) AS [Clé Sage], CASE CT_Type WHEN 0 THEN ''Client'' WHEN 1 THEN ''Fournisseur'' ELSE ''Autre'' END AS [Type tiers] FROM F_COMPTET;',
N'Info_Libres_Cle_Tiers', NULL, 'full', NULL, 'high', 50000, 2, 1);
