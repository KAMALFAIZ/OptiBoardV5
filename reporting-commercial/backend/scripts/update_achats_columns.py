"""
=============================================================================
  UPDATE DES 7 DOCUMENTS ACHATS
  - Corriger orthographe des headers (accents français)
  ...
  Champs à ajouter (masqués) :
    ...
    li.[Catalogue 2], li.[Catalogue 3], li.[Catalogue 4],
  ...
  Exécution: python scripts/update_achats_columns.py
=============================================================================
"""
import pyodbc
import json
...
# ============================================================================
#  COLONNES MASQUÉES À AJOUTER (communes à tous les 7 documents achats)
# ============================================================================
HIDDEN_COLUMNS_SQL = """
    en.[Souche],
    en.[Statut],
...
ORDER BY li.[Date] DESC, li.[N° Pièce]""",
        "columns": [
            ...
            {"field": "Date", "header": "Date", "width": 110, "type": "date"},
            {"field": "Code Fournisseur", "header": "Code Fournisseur", "width": 120, "visible": False},
            {"field": "Fournisseur", "header": "Fournisseur", "width": 200},
            ...
            {"field": "Remise", "header": "Remise", "width": 90, "type": "number", "format": "#,##0.00"},
            {"field": "Frais Approche", "header": "Frais Approche", "width": 110, "type": "number", "format": "#,##0.00"},
            {"field": "CMUP", "header": "CMUP", "width": 100, "type": "number", "format": "#,##0.00"},
            {"field": "Prix de Revient", "header": "Prix de Revient", "width": 110, "type": "number", "format": "#,##0.00"},
            {"field": "Code Dépôt", "header": "Code Dépôt", "width": 100},
            {"field": "Dépôt", "header": "Dépôt", "width": 140},
            {"field": "Catégorie Comptable", "header": "Catégorie Comptable", "width": 140},
        ]
    },
    {
        "code": "DS_ACH_BL",
    ...
    li.[Code dépôt] AS [Code Dépôt],
    li.[Intitulé dépôt] AS [Dépôt],
    li.[N° Pièce BC] AS [N° BC],{HIDDEN_COLUMNS_SQL}
{BASE_JOIN}
...
ORDER BY li.[Date] DESC, li.[N° Pièce]""",
        "columns": [
            ...
            {"field": "Montant HT", "header": "Montant HT", "width": 120, "type": "number", "format": "#,##0.00"},
            {"field": "Montant TTC", "header": "Montant TTC", "width": 120, "type": "number", "format": "#,##0.00"},
            {"field": "Date Livraison Prévue", "header": "Date Livraison Prévue", "width": 140, "type": "date"},
            {"field": "Code Dépôt", "header": "Code Dépôt", "width": 100},
            {"field": "Dépôt", "header": "Dépôt", "width": 140},
        ]
    },
    {
        ...
        "total_columns": ["Quantité", "Montant HT"],
        "query": f"""SELECT
    li.[societe] AS [Societe],
    li.[N° Pièce] AS [N° Piece],
    li.[Date] AS [Date],
    ...
    li.[Quantité] AS [Quantité],
    li.[Prix unitaire] AS [Prix Unitaire],
    li.[Montant HT Net] AS [Montant HT],{HIDDEN_COLUMNS_SQL}
{BASE_JOIN}
...
ORDER BY li.[Date] DESC, li.[N° Pièce]""",
    ...
    li.[Montant HT Net] AS [Montant HT],
    li.[Montant TTC Net] AS [Montant TTC],
    li.[Code dépôt] AS [Code Dépôt],{HIDDEN_COLUMNS_SQL}
{BASE_JOIN}
...
def main():
    print("=" * 70)
    print("  MISE À JOUR DES 7 DOCUMENTS ACHATS")
    ...
    for doc in DOCUMENTS:
        ...
        if row:
            ...
            updated_gv += 1
            print(f"  [OK] GridView ID={gv_id} colonnes mises à jour ({len(doc['columns'])} visibles + {len(HIDDEN_COLUMNS_COLS(doc))} masquées, totaux: {tc})")
        else:
            print(f"  [SKIP] GridView pour {code} non trouvé")

    # 3. Aussi mettre à jour le rapport "Commandes en Cours Fournisseurs" (DS_ACH_COMMANDES_EN_COURS)
    print(f"\n--- Commandes en Cours Fournisseurs (DS_ACH_COMMANDES_EN_COURS) ---")
    cmd_query = f"""SELECT
    li.[societe] AS [Societe],
    li.[N° Pièce] AS [N° Piece],
    li.[Date] AS [Date Commande],
    li.[Code fournisseur] AS [Code Fournisseur],
    li.[Intitulé fournisseur] AS [Fournisseur],
    li.[Code article] AS [Code Article],
    li.[Désignation] AS [Désignation],
    ...
    en.[Statut],
    DATEDIFF(DAY, li.[Date], GETDATE()) AS [Ancienneté (j)],{HIDDEN_COLUMNS_SQL}
{BASE_JOIN}
WHERE li.[Type Document] = 'Bon de commande'
...
ORDER BY [Ancienneté (j)] DESC"""
    ...
    cursor.execute(
        "UPDATE APP_DataSources_Templates SET query_template = ? WHERE code = ?",
        (cmd_query, "DS_ACH_COMMANDES_EN_COURS")
    )
    if cursor.rowcount > 0:
        updated_ds += 1
        print(f"  [OK] Requête SQL mise à jour")
    else:
        print(f"  [SKIP] Template DS_ACH_COMMANDES_EN_COURS non trouvé")

    # Mettre à jour les colonnes du gridview Commandes en Cours
    cursor.execute(
        "SELECT id FROM APP_GridViews WHERE data_source_code = ?",
        ("DS_ACH_COMMANDES_EN_COURS",)
    )
    row = cursor.fetchone()
    if row:
        gv_id = row[0]
        cmd_cols = [
            ...
            {"field": "Qté Reçue", "header": "Qté Reçue", "width": 90, "type": "number"},
            {"field": "Qté Restante", "header": "Qté Restante", "width": 100, "type": "number"},
            {"field": "Prix Unitaire", "header": "Prix Unitaire", "width": 110, "type": "number", "format": "#,##0.00"},
            {"field": "Montant HT", "header": "Montant HT", "width": 120, "type": "number", "format": "#,##0.00"},
            {"field": "Statut Livraison", "header": "Statut Livraison", "width": 120},
            {"field": "Statut", "header": "Statut", "width": 100},
            {"field": "Ancienneté (j)", "header": "Ancienneté (j)", "width": 100, "type": "number"},
        ] + HIDDEN_GRIDVIEW_COLS
        cmd_tc = ["Qté Commandée", "Qté Reçue", "Qté Restante", "Montant HT"]
        cursor.execute(
            "UPDATE APP_GridViews SET columns_config = ?, show_totals = 1, total_columns = ? WHERE id = ?",
        ...
        print(f"  [SKIP] GridView pour DS_ACH_COMMANDES_EN_COURS non trouvé")
