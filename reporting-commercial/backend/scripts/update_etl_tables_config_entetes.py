...
cur.execute(
    "UPDATE APP_ETL_Tables_Config SET source_query = ?, version = version + 1, date_modification = GETDATE() WHERE code = N'Entête_des_ventes'",
    (NEW_QUERY,)
)
print(f"APP_ETL_Tables_Config -> {cur.rowcount} ligne(s) mise(s) à jour")

# Aussi mettre à jour APP_ETL_Tables_Published (copie publiée vers les clients)
cur.execute(
    "UPDATE APP_ETL_Tables_Published SET source_query = ?, date_publication = GETDATE() WHERE code = N'Entête_des_ventes'",
    (NEW_QUERY,)
)
print(f"APP_ETL_Tables_Published -> {cur.rowcount} ligne(s) mise(s) à jour")

c.close()
print("Done — rafraîchissez la page dans l'UI")
